...
export type CatalogCursor={page:number;catalogDone:boolean;pending:{parent:WooProduct;page:number}[];productsSeen:number;productsDone:number;totalProducts:number;records:number};
export function initialCursor():CatalogCursor{return{page:1,catalogDone:false,pending:[],productsSeen:0,productsDone:0,totalProducts:0,records:0};}
const fields="id,name,sku,type,manage_stock,stock_quantity,stock_status,categories,attributes,variations";
export async function advanceCatalog(storeId:StoreId,credentials:Credentials,previous:CatalogCursor,request:typeof fetch=fetch){
 ...
 if(!cursor.pending.length&&!cursor.catalogDone){
  ...
  cursor.productsSeen+=result.items.length;
  cursor.totalProducts=result.total||cursor.totalProducts;
  cursor.catalogDone=(result.pages>0&&cursor.page>=result.pages)||result.items.length<100;
  cursor.page++;
 ...
 tasks.forEach((task,i)=>{
  ...
  if((result.pages>0&&task.page>=result.pages)||result.items.length<100)cursor.productsDone++;
