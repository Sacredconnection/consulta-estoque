...
export type BulkGroup={key:string;name:string;code:string;search:string;loose:Partial<Record<StoreId,BulkAmount>>;sizes:Partial<Record<BulkSize,Partial<Record<StoreId,BulkAmount>>>>};
...
export function buildBulkGroups(products:Product[],storeIds:StoreId[]):BulkGroup[]{
 ...
 for(const product of products)for(const stock of product.stocks){
  const loose=stock.storeId==='pagnier'&&stock.quantityUnit==='kg';
  const pot=BULK_SIZES.includes(stock.grams as BulkSize)&&(stock.quantityUnit??'un.')==='un.';
  if(!allowed.has(stock.storeId)||!isRape(product,stock)||(!loose&&!pot)||stock.packaging!=='bulk'||stock.shared)continue;
  ...
  if(!group){group={key,name,code,search:'',loose:{},sizes:{}};groups.set(key,group);}
  if(name.length<group.name.length)group.name=name;
  group.search+=' '+normalize([name,product.sku,product.category,code].join(' '));
  const size=stock.grams as BulkSize,values=loose?group.loose:group.sizes[size]??{};
  const known=stock.quantity!==null&&Number.isFinite(stock.quantity);
  const value:BulkAmount={quantity:known?stock.quantity:null,kg:known?Math.max(0,stock.quantity!)*(loose?1:size/1000):null,partial:!known};
  values[stock.storeId]=values[stock.storeId]?sumBulk([values[stock.storeId]!,value]):value;
  if(!loose)group.sizes[size]=values;
 }
...
export function bulkTotal(groups:BulkGroup[],storeId?:StoreId){
 return sumBulk(groups.flatMap(group=>[group.loose,...BULK_SIZES.map(size=>group.sizes[size]??{})].flatMap(values=>storeId?(values[storeId]?[values[storeId]!]:[]):Object.values(values))));
}
