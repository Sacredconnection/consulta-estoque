...
export type ReplenishmentReport={generatedAt:string;lastSync:string|null;source:string;lines:ReplenishmentLine[];warning?:string;storeId?:StoreId;storeName?:string;configured?:boolean};
export function sacredReplenishment(minima:SacredMinimum[],products:Product[],storeId:StoreId='sacred'):ReplenishmentLine[]{
 ...
 for(const p of products){const key=p.sku.trim().toUpperCase();const stocks=index.get(key)??[];stocks.push(...p.stocks.filter(s=>s.storeId===storeId).map(s=>({...s,productName:s.productName??p.name})));index.set(key,stocks);}
