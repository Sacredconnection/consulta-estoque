...
export async function state(){
 const db=database();
 const [rows,rule]=await Promise.all([db.prepare("SELECT id,snapshot,last_sync,error FROM connections").all<Omit<Connection,"credentials">>(),getRule()]);
 ...
 return {demo:rows.results.length===0,products,rule,connections:STORES.map(s=>{const c=rows.results.find(r=>r.id===s.id);return{id:s.id,connected:!!c,lastSync:c?.last_sync??null,error:c?.error??null};})};
}
async function syncStore(storeId:StoreId){
 ...
 try{
  ...
  const records=await readCatalog(storeId,await unseal(storeId,connection.credentials),at);
