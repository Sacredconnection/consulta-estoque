...
export function availableCategories(products:Product[],storeId:StoreId):string[]{
 ...
 for(const p of products)for(const s of p.stocks.filter(s=>s.storeId===storeId))for(const c of categoryKeys(stockCategoryPaths(p,s)))categories.set(normalize(c),c);
...
export function filterCategory(products:Product[],storeId:StoreId,category:string|string[]):Product[]{
 ...
 return products.map(p=>({...p,stocks:p.stocks.filter(s=>s.storeId===storeId&&(!targets.size||categoryKeys(stockCategoryPaths(p,s)).some(c=>targets.has(normalize(c)))))})).filter(p=>p.stocks.length);
}
