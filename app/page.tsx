...
export default function Home(){
 ...
 const category=singleStore&&categorySelection?.storeId===singleStore&&categories.includes(categorySelection.value)?categorySelection.value:'';
 selectedCategoryRef.current=category||undefined;
 useEffect(()=>setCategorySelection(null),[singleStore]);
 const catalogRows=useMemo(()=>buildStockRows(category&&singleStore?filterCategory(catalogProducts,singleStore,category):scopeProductsToStores(catalogProducts,storeIds)),[catalogProducts,storeIds,category,singleStore]);
 ...
 async function ask(text=message){
  if(!text.trim()||chatBusy||!storeIds.length)return;setExpandedQueries([]);setMessage("");setMessages(m=>[...m,{role:"user",text,storeIds:[...storeIds],category:category||undefined}]);setChatBusy(true);
  try{const r=await fetch("/api/agent",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({message:text,storeIds,category:category||undefined})});
   const d=await r.json() as {error?:string;text:string;products?:Product[]};if(!r.ok)throw Error(d.error||"Consulta indisponível");
   setMessages(m=>[...m,{role:"assistant",text:d.text,products:d.products,storeIds:[...storeIds],category:category||undefined}]);
