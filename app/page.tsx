...
export default function Home(){
 ...
 const companyFilter=connections.length>0&&<fieldset className="company-filter" disabled={chatBusy}><legend>Empresas nas buscas</legend><div className="company-options">{connections.map(c=>{const store=STORES.find(s=>s.id===c.id)!;return <label key={c.id}><input type="checkbox" checked={storeIds.includes(c.id)} onChange={()=>setExcludedStores(current=>current.includes(c.id)?current.filter(id=>id!==c.id):[...current,c.id])}/><span className="store-dot" style={{background:store.color}}/>{store.short}</label>;})}<button type="button" onClick={()=>setExcludedStores([])} disabled={storeIds.length===connections.length}>Selecionar todas</button></div><p>{storeIds.length?`${storeIds.length} de ${connections.length} selecionadas. O filtro vale para novas consultas e para o catálogo.`:'Selecione pelo menos uma empresa para consultar.'}</p>{singleStore&&<div className="category-filter"><label htmlFor="category-select">Categorias · {STORES.find(s=>s.id===singleStore)!.short}</label><select id="category-select" aria-label="Adicionar categoria" value="" onChange={e=>{if(e.target.value)setCategorySelection({storeId:singleStore,value:[...category,e.target.value]});}} disabled={chatBusy||!categories.some(c=>!category.includes(c))}><option value="">{category.length?'Adicionar outra categoria…':'Todas as categorias · selecione para filtrar'}</option>{categories.filter(c=>!category.includes(c)).map(name=><option key={name} value={name}>{name}</option>)}</select>{category.length>0&&<><div className="category-chips">{category.map(name=><button type="button" key={name} aria-label={'Remover categoria '+name} onClick={()=>setCategorySelection({storeId:singleStore,value:category.filter(c=>c!==name)})}>{name}<X size={14}/></button>)}</div><button className="category-clear" type="button" onClick={()=>setCategorySelection(null)}>Limpar categorias</button><small>Inclui produtos de qualquer uma das categorias selecionadas.</small></>}{!categories.length&&<small>Categorias disponíveis após a sincronização.</small>}</div>}</fieldset>;
