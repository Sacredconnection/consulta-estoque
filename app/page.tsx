...
type Message={role:"user"|"assistant";text:string;products?:Product[];storeIds?:StoreId[];category?:string[]};
...
export default function Home(){
 ...
 const [categorySelection,setCategorySelection]=useState<{storeId:StoreId;value:string[]}|null>(null);
 ...
 const [activeView,setActiveView]=useState<"assistant"|"catalog"|"replenishment"|"tracking"|"bulk">("assistant"),[catalogProducts,setCatalogProducts]=useState<Product[]>([]);
 ...
 const singleStore=storeIds.length===1?storeIds[0]:undefined;
 const categories=useMemo(()=>singleStore?availableCategories(catalogProducts,singleStore):[],[catalogProducts,singleStore]);
 const category=useMemo(()=>singleStore&&categorySelection?.storeId===singleStore?categorySelection.value.filter(c=>categories.includes(c)):[],[singleStore,categorySelection,categories]);
 selectedCategoryRef.current=category.length?category:undefined;
 useEffect(()=>setCategorySelection(null),[singleStore]);
 const catalogRows=useMemo(()=>buildStockRows(category.length&&singleStore?filterCategory(catalogProducts,singleStore,category):scopeProductsToStores(catalogProducts,storeIds)),[catalogProducts,storeIds,category,singleStore]);
 ...
 useEffect(()=>setCatalogPage(1),[catalogQuery,storeIds,category]);
 ...
 async function ask(text=message){
  if(!text.trim()||chatBusy||!storeIds.length)return;setExpandedQueries([]);setMessage("");setMessages(m=>[...m,{role:"user",text,storeIds:[...storeIds],category:category.length?category:undefined}]);setChatBusy(true);
  try{const r=await fetch("/api/agent",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({message:text,storeIds,category:category.length?category:undefined})});
   const d=await r.json() as {error?:string;text:string;products?:Product[]};if(!r.ok)throw Error(d.error||"Consulta indisponível");
   setMessages(m=>[...m,{role:"assistant",text:d.text,products:d.products,storeIds:[...storeIds],category:category.length?category:undefined}]);
 ...
 return <div className="chat-app">
  ...
  <main className={"chat-main "+(isLanding?"landing-main ":"")+(activeView!=="assistant"?"catalog-active":"")}>
   ...
   {!isLanding&&activeView!=="replenishment"&&activeView!=="tracking"&&activeView!=="bulk"&&companyFilter}
   <nav className="view-tabs" role="tablist" aria-label="Visualização do estoque">
    ...
    <button id="bulk-tab" role="tab" aria-selected={activeView==="bulk"} aria-controls="bulk-panel" onClick={()=>setActiveView("bulk")}>Granel simplificado</button>
   ...
   {activeView==="tracking"&&<div id="tracking-panel" role="tabpanel" aria-labelledby="tracking-tab"><Tracking/></div>}
   {activeView==="bulk"?<div id="bulk-panel" role="tabpanel" aria-labelledby="bulk-tab"><details className="bulk-filters"><summary>Empresas e categorias · {storeIds.length} selecionada(s){category.length?" · "+category.join(" / "):""}</summary>{companyFilter}</details><BulkSummary products={category.length&&singleStore?filterCategory(catalogProducts,singleStore,category):catalogProducts} storeIds={storeIds} ready={ready}/></div>:activeView==="assistant"?<div id="assistant-panel" role="tabpanel" aria-labelledby="assistant-tab">{!messages.length?<section className="welcome search-welcome"><div className="eyebrow">ESTOQUE CONSOLIDADO</div><h1>Consulte seu estoque</h1><p>Busque por produto ou código nas empresas selecionadas.</p><div className="search-start">{companyFilter}{composer}<p className="search-example"><span><code>RAYA02</code> família completa</span><span><code>RAYA02-10</code> apresentação específica</span></p>{syncSummary}</div></section>:<section className="conversation" aria-label="Conversa com o agente" aria-live="polite" aria-relevant="additions">{messages.map((m,i)=>{if(m.role!=="user")return null;const response=messages[i+1];return <QueryTurn key={i} question={m.text} index={i} historical={i!==latestQueryIndex} expanded={expandedQueries.includes(i)} onToggle={()=>setExpandedQueries(current=>current.includes(i)?current.filter(index=>index!==i):[...current,i])}>{response?.role==="assistant"&&<article className="message assistant"><div className="speaker"><Network size={17}/>MS Lumiar · Agente de estoque</div><p className="query-companies">Empresas: {(response.storeIds??storeIds).map(id=>STORES.find(s=>s.id===id)!.short).join(" / ")}{response.category?.length?" · Categorias: "+response.category.join(" / "):""}</p>{response.products?.length?<StockTable rows={buildStockRows(response.products)} storeIds={response.storeIds??storeIds} rule={rule} title="Resultado em tabela" exportContext={m.text+(response.category?.length?" · Categorias: "+response.category.join(" / "):"")}/>:<Answer text={response.text}/>}</article>}</QueryTurn>;})}{chatBusy&&<div className="thinking" role="status"><LoaderCircle size={17} className="spin"/>Consultando o último estoque sincronizado…</div>}</section>}</div>:activeView==="replenishment"?<div id="replenishment-panel" role="tabpanel" aria-labelledby="replenishment-tab"><Replenishment storeIds={connections.map(c=>c.id)}/></div>:activeView==="catalog"?<section id="catalog-panel" className="catalog-view" role="tabpanel" aria-labelledby="catalog-tab"><header className="catalog-header"><div className="eyebrow">CATÁLOGO CONSOLIDADO</div><h1>Todos os dados</h1><p>Consulte produtos, apresentações e SKUs disponíveis em todas as lojas sincronizadas.</p></header><label className="catalog-search"><Search size={18}/><span className="sr-only">Buscar no catálogo</span><input value={catalogQuery} onChange={event=>setCatalogQuery(event.target.value)} placeholder="Buscar por SKU, produto, apresentação ou categoria…"/>{catalogQuery&&<button type="button" onClick={()=>setCatalogQuery("")} aria-label="Limpar busca"><X size={17}/></button>}</label>{!ready?<div className="catalog-empty"><LoaderCircle size={18} className="spin"/>Carregando catálogo…</div>:filteredCatalogRows.length?<><div className="catalog-summary"><strong>{filteredCatalogRows.length.toLocaleString("pt-BR")} SKU(s)</strong><span>Exibindo {(catalogStart+1).toLocaleString("pt-BR")}–{Math.min(catalogStart+CATALOG_PAGE_SIZE,filteredCatalogRows.length).toLocaleString("pt-BR")}</span></div><StockTable totalRows={filteredCatalogRows} rows={visibleCatalogRows} storeIds={storeIds} rule={rule} exportContext={(catalogQuery||"Todos os dados")+(category.length?" · Categorias: "+category.join(" / "):"")}/>{catalogPages>1&&<nav className="catalog-pages" aria-label="Paginação do catálogo"><button type="button" disabled={currentCatalogPage===1} onClick={()=>setCatalogPage(currentCatalogPage-1)}><ArrowLeft size={16}/>Anterior</button><span>Página {currentCatalogPage.toLocaleString("pt-BR")} de {catalogPages.toLocaleString("pt-BR")}</span><button type="button" disabled={currentCatalogPage===catalogPages} onClick={()=>setCatalogPage(currentCatalogPage+1)}>Próxima<ArrowRight size={16}/></button></nav>}<p className="table-note">* Total parcial. “comp.” indica saldo compartilhado entre variações. Passe o cursor sobre a célula para ver detalhes.</p></>:<div className="catalog-empty">Nenhum SKU corresponde à consulta.</div>}</section>:null}
   <div ref={bottom}/>
  </main>
