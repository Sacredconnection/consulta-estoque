...
Trocar a empresa ou selecionar várias limpa a categoria. Consultas anteriores preservam suas empresas, categoria e resultados. O filtro também vale para Todos os dados e não interfere no pedido de reposição.

Cada resultado oferece Exportar Excel. No catálogo paginado, a exportação inclui todas as páginas filtradas. A planilha contém a consulta, SKU, produto, apresentação, quantidades e unidades por empresa, SKUs de origem, kg e totais, com observações para saldos desconhecidos, compartilhados e parciais. Valores numéricos são números; nomes e códigos são texto literal. A exportação usa o resultado exibido, sem iniciar nova sincronização.
