...
export function BulkSummary({products,storeIds,ready}:{products:Product[];storeIds:StoreId[];ready:boolean}){
 ...
 return <section className="bulk-view">
  ...
  {!ready?<p className="bulk-empty" role="status">Carregando estoque…</p>:!storeIds.length?<p className="bulk-empty">Selecione uma empresa nos filtros.</p>:!visible.length?<p className="bulk-empty">{query?'Nenhum produto corresponde à busca.':'Nenhum pote de 100 g, 250 g ou 500 g no estoque carregado.'}</p>:<>
   <div className="bulk-scroll" tabIndex={0} role="region" aria-label="Estoque de granel por empresa"><table className="bulk-table">
    <caption className="sr-only">Potes de 100, 250 e 500 gramas em unidades. A primeira linha soma seu peso em quilogramas; cada produto também mostra seu subtotal em quilogramas.</caption>
