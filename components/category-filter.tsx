...
export function CategoryFilter({categories,selected,onChange,company,disabled=false,id,compact=false}:{categories:string[];selected:string[];onChange:(values:string[])=>void;company:string;disabled?:boolean;id:string;compact?:boolean}){
 ...
 return <fieldset className={'category-filter category-hierarchy'+(compact?' category-filter-compact':'')} disabled={disabled}><legend className={compact?'sr-only':undefined}>Categorias · {company}</legend>
  ...
  {!compact&&<small>{roots.length?'A categoria pai inclui todas as subcategorias. Selecione subcategorias para refinar; adicione outras para acumular.':'Categorias disponíveis após carregar a hierarquia da empresa.'}</small>}
 </fieldset>;
}
