...
export function Replenishment({storeIds}:{storeIds:StoreId[]}){
 ...
 const filteredReport=report?filterReplenishmentReport(report,selectedCategories):null;
 const order=filteredReport?.lines.filter(r=>r.status==='order')??[],review=filteredReport?.lines.filter(r=>r.status==='review')??[];
 const lines=view==='order'?order:view==='review'?review:filteredReport?.lines??[];
 async function save(format:'pdf'|'xlsx'){if(!filteredReport)return;setExporting(true);try{const {exportReplenishment}=await import('@/lib/replenishment-export');await exportReplenishment(filteredReport,format);}catch{setError('Não foi possível exportar. Tente novamente.');}finally{setExporting(false);}}
 return <section className="replenishment-view"><header><div className="eyebrow">POR EMPRESA</div><h1>Reposição de Estoque</h1><p>Pedido calculado com os mínimos e o estoque da empresa selecionada.</p></header><label className="replenishment-company">Empresa<select value={storeId} disabled={exporting} onChange={event=>{setReport(null);setError('');setSelectedCategories([]);setStoreId(event.target.value as StoreId);}}>{STORES.filter(s=>s.id==='sacred'||storeIds.includes(s.id)).map(s=><option key={s.id} value={s.id}>{s.short}</option>)}</select></label>
 {report&&report.configured!==false&&<CategoryFilter key={storeId} id="replenishment-category" company={company} categories={categories} selected={selectedCategories} disabled={busy||exporting} onChange={setSelectedCategories}/>}
 ...
 <p className="table-note">Repor = mínimo − disponível. Saldos negativos contam como zero. Itens sem saldo confiável ficam para revisão e não entram no pedido. Os arquivos incluem o pedido das categorias selecionadas, independentemente da visualização abaixo.</p>
