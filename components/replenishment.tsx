 ...
 return <section className="replenishment-view">
  ...
  {report&&report.configured!==false&&<>
   <div className="replenishment-metrics" aria-label="Resumo do pedido"><div className="replenishment-metric"><span>Produtos a repor</span><strong>{number(order.length)}</strong><small>SKUs no pedido</small></div>
    <div className="replenishment-metric"><span>{storeId==='pagnier'?'Reposição por medida':'Unidades'}</span><strong>{storeId==='pagnier'?['kg','L'].map(unit=>number(order.filter(r=>r.unit===unit).reduce((n,r)=>n+r.order!,0))+' '+unit).join(' / '):number(order.reduce((n,r)=>n+r.order!,0))}</strong><small>Quantidade para reposição</small></div>
