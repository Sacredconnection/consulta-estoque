...
export function StockTable({rows,storeIds,rule,title,totalRows=rows}:{rows:StockRow[];storeIds:StoreId[];rule:Rule;title?:string;totalRows?:StockRow[]}){
 ...
 return <section className="stock-table-section">
  ...
  <div className="stock-table-scroll"><table className="stock-table" style={{minWidth:Math.max(820,520+storeIds.length*210)}}>
   ...
   <thead><tr><th rowSpan={2}>SKU</th><th rowSpan={2}>Produto</th><th rowSpan={2}>Apresentação</th><th rowSpan={2}>Categoria</th>{storeIds.map(id=>{const store=STORES.find(s=>s.id===id)!;return <th key={id} colSpan={2} className="numeric store-column store-start store-band" style={storeStyle(id)}><span className="store-heading"><i style={{background:store.color}}/>{store.short}</span></th>;})}<th rowSpan={2} className="numeric kg-total">Total kg</th></tr><tr>{storeIds.map(id=><StoreHead key={id} storeId={id}/>)}</tr></thead>
   ...
   <tfoot><tr><th colSpan={4}>Total kg · todos os resultados{totalRows.length!==rows.length?' (todas as páginas)':''}</th>{totals.map((mass,i)=><TotalCells key={storeIds[i]} storeId={storeIds[i]} text={kg(mass)}/>)}<td className="numeric kg-total"><strong>{kg(grand)}</strong></td></tr>
   <tr><th colSpan={4}>Dos quais, granel (kg)</th>{totals.map((mass,i)=><TotalCells key={storeIds[i]} storeId={storeIds[i]} text={mass.bulkKg===null?'—':number(mass.bulkKg)+(mass.partial?' *':'')}/>)}<td className="numeric kg-total">{grand.bulkKg===null?'—':number(grand.bulkKg)+(grand.partial?' *':'')}</td></tr></tfoot>
  </table></div>
