...
import {RefreshCw,Mail,Upload} from 'lucide-react';
import type {TrackedShipment} from '@/lib/tracking-service';

type Data={rows:TrackedShipment[];enabled:boolean;interval:number;importedAt:string|null;source:string;integration?:{provider:string;configured:boolean};carriers:{name:string;ready:boolean}[]};
...
export function Tracking(){
 ...
 const rows=useMemo(()=>(data?.rows??[]).filter(r=>(historical||!r.historical)&&[r.order,r.customer,r.tracking,r.carrier,r.result?.status].join(' ').toLowerCase().includes(query.toLowerCase())),[data,historical,query]);
 const current=Math.min(page,Math.max(1,Math.ceil(rows.length/50))),visible=rows.slice((current-1)*50,current*50);
 const active=data?.rows.filter(r=>!r.historical)??[];
 return <section className="tracking-view" aria-labelledby="tracking-title">
  ...
  <div className="tracking-toolbar"><label className="tracking-search"><input aria-label="Buscar pedido, cliente ou tracking" placeholder="Buscar pedido, cliente ou tracking" value={query} onChange={e=>{setQuery(e.target.value);setPage(1);}}/></label><label><input type="checkbox" checked={historical} onChange={e=>{setHistorical(e.target.checked);setPage(1);}}/> Incluir histórico e cancelados</label><button className="tracking-email" disabled><Mail size={16}/>E-mails · em breve</button></div>
  <div className="tracking-table-wrap"><table className="tracking-table"><thead><tr>{['Pedido / origem','Cliente','Transportadora / tracking','Status anotado na planilha','Entrega prevista','Rastreio na transportadora','Última consulta válida',''].map((label,i)=><th key={i}>{label}</th>)}</tr></thead><tbody>{visible.map(r=><tr key={r.id}><td>{r.order}<br/><small>{r.sheet} · linha {r.row}</small></td><td>{r.customer}</td><td>{r.carrier}<br/><code>{r.tracking||'Sem tracking'}</code></td><td>{r.orderStatus||'Não informado'}</td><td>{delivery(r.result?.expectedDelivery??null)}{r.result?.expectedDeliverySource&&<><br/><small>{r.result.expectedDeliverySource==='Official'?'Previsão da transportadora':'Estimativa '+r.result.expectedDeliverySource}</small></>}</td><td><strong>{r.result?.status??'Não consultado'}</strong><br/>{r.result?.description}{r.result?.location&&<><br/>{r.result.location}</>}<br/><small>{r.issue||r.error||(!data?.carriers.find(c=>c.name===r.carrier)?.ready?' Integração pendente':'')}</small></td><td>{time(r.result?.checkedAt??null)}{r.result?.source&&<><br/><small>Via {r.result.source}</small></>}{r.result?.eventAt&&<><br/><small>Evento: {time(r.result.eventAt)}</small></>}{r.result?.carrierSyncedAt&&<><br/><small>Coleta na transportadora: {time(r.result.carrierSyncedAt)}</small></>}</td><td><button className="tracking-refresh" disabled={busy||!!r.issue||!data?.carriers.find(c=>c.name===r.carrier)?.ready} onClick={()=>void change({action:'refresh',id:r.id})} aria-label={'Consultar pedido '+r.order}><RefreshCw size={14}/></button></td></tr>)}</tbody></table>{!visible.length&&<p className="tracking-empty">{data?'Nenhum envio nesta seleção.':'Carregando envios…'}</p>}</div>
